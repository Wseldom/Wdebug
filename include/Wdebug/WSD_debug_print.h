...
#include <stdio.h>
		...
		#define		WSD_DEBUG_BUF(FORMAT,BUF,LENGTH)
		...
		#define		WSD_DEBUG_BUF(FORMAT,BUF,LENGTH)		printf(Trmnl_RST WSD_DEBUG_HEAD_ATTR "[文件:%s][时间:%s][日期:%s][函数:%s][行:%d]" "buf--->{\r\n" WSD_DEBUG_INFO_ATTR ,__FILE__, __TIME__, __DATE__, __func__, __LINE__ ); \
		...
		#define		WSD_DEBUG_INFO(info,...)		printf(Trmnl_RST WSD_DEBUG_HEAD_ATTR "[文件:%s][函数:%s][行:%d]" "调试信息--->[" WSD_DEBUG_INFO_ATTR  info Trmnl_SET_FORE(Trmnl_color_yellow) "]\r\n" Trmnl_RST, __FILE__, __func__, __LINE__, ##__VA_ARGS__);

		#define		WSD_DEBUG_BUF(FORMAT,BUF,LENGTH)		printf(Trmnl_RST WSD_DEBUG_HEAD_ATTR "[文件:%s][函数:%s][行:%d]" "buf--->{\r\n" WSD_DEBUG_INFO_ATTR ,__FILE__, __func__, __LINE__ ); \
			int WSD_DEBUG_BUF_LEN; \
			for (WSD_DEBUG_BUF_LEN = 0; WSD_DEBUG_BUF_LEN < LENGTH; ++WSD_DEBUG_BUF_LEN) { \
				printf (FORMAT "  ", BUF[WSD_DEBUG_BUF_LEN]); \
			}\
